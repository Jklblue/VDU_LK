...
class VanillaNet(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, dims=[96, 192, 384, 768, 1536],
                 drop_rate=0, act_num=3, strides=[2, 2, 2, 2], deploy=False, ada_pool=None, **kwargs):
        super().__init__()
        self.deploy = deploy
        if self.deploy:
        ...
        self.channel = [i.size(1) for i in self.forward(torch.zeros(1, 3, 1024, 1024))]
...
def vanillanet_L(pretrained='', **kwargs):
    model = VanillaNet(
        dims=[64,256, 512, 1024, 2048],
        strides=[2, 2, 2, 2],  # 步幅配置
        **kwargs
    )
    if pretrained:
        weights = torch.load(pretrained)['model_ema']
    ...
    if pretrained:
        model.load_state_dict(
            model_zoo.load_url('https://github.com/huawei-noah/VanillaNet/releases/download/ckpt/vanillanet_5.pth', model_dir='model_data'),
            strict=False)
